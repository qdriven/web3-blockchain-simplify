...
每一门语言都会有类型系统，这个系统是非常重要的，但是由于类型系统比较繁杂，有些不是很常用，所以建议先通过常用的类型
动手练习,solidity中常用的类型包括了:
...
我们只介绍一些常用的类型，不常用的不讲。这篇介绍数值类型，函数和映射类型在后续中在讲述。
...
<QuizForm lessonId="b4cfc02e-d122-407a-83f3-a7b6aa441e49"></QuizForm>