...
# Solidity: 6. 变量数据存储和作用域 storage/memory/calldata
...
重点是`storage`, `memory`和`calldata`三个关键字的用法。
他们出现的原因是为了节省链上有限的存储空间和降低`gas`。
同时还提到了```solidity```使用中的全部变量：
1. msg.sender: 发起交易地址
2. block: 当前块内容
3. msg.data: 当前发起交易的数据
